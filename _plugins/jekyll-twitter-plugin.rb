...
# httpsgithub.comrob-murrayjekyll-twitter-plugin
#
module TwitterJekyll
  VERSION = 2.0.0.freeze
  REFER_TO_README   = Please see 'httpsgithub.comrob-murrayjekyll-twitter-plugin' for usage..freeze
  ...
  class ApiClient
    ...
    def fetch(api_request)
      ...
      response = NetHTTP.start(uri.host, use_ssl api_request.ssl) do http
        http.read_timeout = 5
        http.open_timeout = 5
  ...
  ErrorResponse = Struct.new(request, message) do
    def html
      pThere was a '#{message}' error fetching URL '#{request.entity_url}'p
    end
  ...
  ApiRequest = Struct.new(entity_url, params) do
    TWITTER_API_URL = httpspublish.twitter.comoembed.freeze
    ...
    def cache_key
      DigestMD5.hexdigest(#{self.class.name}-#{unique_request_key})
    end
  ...
  # @api public
  class TwitterTag  LiquidTag
    ERROR_BODY_TEXT = pTweet could not be processedp.freeze
    ...
    def arguments_deprecation_warning(args)
      warn #{LIBRARY_VERSION} Passing '#{OEMBED_ARG}' as the first argument is not required anymore. This will result in an error in future versions.nCalled with #{args.inspect}
    end
...
LiquidTemplate.register_tag(twitter, TwitterJekyllTwitterTag)
LiquidTemplate.register_tag(twitternocache, TwitterJekyllTwitterTagNoCache)